...
if choice == 'Business Objective': 
    ...
    st.markdown('Dữ liệu được cung cấp sẵn trong tập tin 2_Reviews.csv với gần 30.000 mẫu gồm các thông tin:')
    st.write('ID (mã), User (người dùng), Time (thời gian đánh giá), Rating (điểm đánh giá), Comment (nội dung đánh giá), và IDRestaurant (mã nhà hàng)')
    st.markdown('Tập tin chứa thông tin về nhà hàng: 1_Restaurants.csv với hơn 1.600 mẫu gồm các thông tin:')
    st.write('ID (mã), Restaurant (tên nhà hàng), Address (địa chỉ), Time (giờ mở cửa), Price (khoảng giá), District(quận)')
    st.markdown("##")
